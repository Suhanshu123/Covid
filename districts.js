...
window.onload = function () {
    ...
    stateSelect.length = 1; // remove all options
    districtSelect.length = 1; // remove all options
    if (this.selectedIndex < 1) return;
    for (var state in stateObject[this.value]) {
        stateSelect.options[stateSelect.options.length] = new Option(state, state);
        }
    }
    countySelect.onchange(); // reset page in reloaded
    stateSelect.onchange = function () {
        districtSelect.length = 1; // remove all options
        if (this.selectedIndex < 1) return;
        var district = stateObject[countySelect.value][this.value];
...
//var button = document.getElementById('submit').addEventListener('click', buttonClick);

function buttonClick(){
    var obj = document.getElementById("countySelect");
    var obj1 = document.getElementById("stateSelect");
    var obj2 = document.getElementById("districtSelect");

    document.write(obj.options[obj.selectedIndex].text);
    document.write(obj1.options[obj1.selectedIndex].text);
    document.write(obj2.options[obj2.selectedIndex].text);
//      document.getElementById("county").textContent = obj.options[obj.selectedIndex].text;
      document.getElementById("county").textContent = "changed";
//      document.getElementById("county").textContent = obj;
